...
export default function FrameGifManager({ adminToken }) {
  ...
  const loadFrames = async () => {
    try {
      const backendFrames = await backendRequest('/api/admin/frames_gif', adminToken);
  ...
  const handleCreateBlankFrame = async (name, slots = []) => {
    try {
      ...
      await backendRequest('/api/admin/frames_gif', adminToken, {
        method: 'POST',
      ...
      alert('Gagal membuat bingkai GIF kosong: ' + e.message);
    }
  };
  ...
  const handleToggleActive = async (frame) => {
    try {
      ...
      await backendRequest(`/api/admin/frames_gif/${rawId}`, adminToken, {
        method: 'PUT',
        body: JSON.stringify(payload)
      });
      loadFrames();
  ...
  return (
    <div className="flex flex-col h-full bg-[#f1f2f4] overflow-hidden p-6 rounded-xl border border-gray-200 shadow-sm">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 m-0">Manajemen Bingkai GIF</h2>
...
function AddAssetModal({ onClose, onAssetSelected, adminToken }) {
  ...
  useEffect(() => {
    backendRequest('/api/admin/frames_gif', adminToken).then(res => {
      setAssets(res.data || []);
  ...
  const handleUpload = async (file) => {
    ...
    try {
      ...
      const res = await fetch(`${getBackendApiUrl()}/api/admin/frames_gif/upload`, {
        method: 'POST',
